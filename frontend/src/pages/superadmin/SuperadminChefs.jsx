...
const SuperadminChefs = () => {
    ...
    useEffect(() => {
        const fetchChefs = async () => {
            setLoading(true);
            try {
                const res = await fetch(`${API_URL}/superadmin/chefs?page=${page}&limit=20&city=${cityFilter}&status=${statusFilter}&search=${debouncedSearch}`, {
        ...
        <div>
            ...
            <div className="sa-card" style={{ marginBottom: '20px', display: 'flex', gap: '16px', alignItems: 'center' }}>
                <div style={{ flex: 1, position: 'relative' }}>
                    ...
                    <input type="text" placeholder="Search chefs..." className="sa-input" style={{ paddingLeft: '40px', width: '100%' }} value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
                </div>
