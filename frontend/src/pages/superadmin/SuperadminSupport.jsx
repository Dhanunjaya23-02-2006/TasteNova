...
const SuperadminSupport = () => {
    ...
    useEffect(() => {
        const fetchTickets = async () => {
            setLoading(true);
            try {
                const res = await fetch(`${API_URL}/superadmin/support?page=${page}&limit=20&city=${cityFilter}&status=${statusFilter}`, {
        ...
        <div>
            ...
            <div className="sa-card" style={{ marginBottom: '20px', display: 'flex', gap: '16px', alignItems: 'center' }}>
                <div style={{ flex: 1, position: 'relative' }}>
                    ...
                    <input type="text" placeholder="Search tickets by ID or customer..." className="sa-input" style={{ paddingLeft: '40px', width: '100%' }} disabled />
                </div>
