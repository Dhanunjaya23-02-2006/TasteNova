            ...
            {(stats.pendingChefs > 0 || stats.pendingRefunds > 0) && (
                <div className="sa-card" style={{ marginBottom: '24px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                    {stats.pendingChefs > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => navigate('/Admin/chefs?status=pending')}>
                            <span className="sa-badge sa-badge-yellow">{stats.pendingChefs} chef{stats.pendingChefs > 1 ? 's' : ''} awaiting approval</span>
                        </div>
                    )}
                    {stats.pendingRefunds > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => navigate('/Admin/refunds')}>
                ...
                {liveOrders.length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
                        {liveOrders.map(order => (
                            <div key={order._id} className="sa-card" style={{ cursor: 'pointer' }} onClick={() => navigate(`/Admin/orders`)}>
            ...
            {recentTickets.length > 0 && (
                <div>
                    ...
                    <table className="sa-table">
                        ...
                        <tbody>
                            {recentTickets.map(t => (
                                <tr key={t._id} style={{ cursor: 'pointer' }} onClick={() => navigate('/Admin/support')}>
