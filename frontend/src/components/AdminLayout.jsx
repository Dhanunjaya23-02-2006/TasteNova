...
const moreNav = [
    ...
    { to: '/admin/city-settings', label: 'City Settings', icon: Settings },
];
...
const AdminLayout = () => {
    ...
    useEffect(() => {
        if (!user || !['Admin', 'admin', 'superadmin'].includes(user.role)) {
            navigate('/');
        }
    }, [user, navigate]);
    ...
    if (!user || !['Admin', 'admin', 'superadmin'].includes(user.role)) return null;
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-dark)' }}>
            ...
            <header className="subadmin-topbar">
                <div className="subadmin-topbar-left">
                    ...
                    <NavLink to="/Admin" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        {userOpen && (
                            <div className="subadmin-dropdown-menu" style={{ right: 0, left: 'auto', minWidth: '220px' }}>
                                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-subtle)' }}>
                                    ...
                                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Regional Admin</div>
                                </div>
                                <NavLink to="/admin/profile" className="subadmin-dropdown-item" onClick={() => setUserOpen(false)}>
                                    <User size={16} /> My Profile
                                </NavLink>
                                <NavLink to="/admin/assigned-zones" className="subadmin-dropdown-item" onClick={() => setUserOpen(false)}>
                                    <MapPin size={16} /> Assigned Zones
                                </NavLink>
                                <NavLink to="/Admin/permissions" className="subadmin-dropdown-item" onClick={() => setUserOpen(false)}>
